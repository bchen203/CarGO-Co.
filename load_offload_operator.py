...
class Load_Offload_Operator():
    ...
    # Move on
        ...
        while(True): #break on finding a solution
            ...
            for container in movable_containers:
                ...
                if(self.is_in_offloads(container)): #for offloads
            ...
            load_destinations = [] #for loads
            for column in range(len(self.calculator.ship_bay_array[0])):
                curContainer = calculate.get_supported_empty_space(self.calculator.ship_bay_array, column)
                if not curContainer == False: #False if no valid empty spaces
                    load_destinations.append(curContainer)

            for container in load_destinations:
    ...
    def is_load_offload_possible(manifest_array,loader):
        num_Offloads = 0
        num_Loads = 0
    ...
    def is_finished_transferring(loader):
        containers_left = 0
    ...
    def get_truck_container(loader):
        load_list = loader.get_pending_loads()
        if(load_list):
            for key in load_list:
                return key
    ...
    def follow_instruction(self,instruction,loader):
        ...
        if(instruction.starting_location[0] == 8 and instruction.starting_location[1] == 0): 
            #instruction is load
            current_container = manifest.Container(0,self.get_truck_container(loader),1 )
            self.loading_stack.push(current_container)
            ...
            current_container = self.calculator[instruction.starting_location[0]][instruction.starting_location[1]]
            self.offloading_stack.push(current_container)
    ...
    def follow_reverse_instruction(self,instruction,loader):
        if(instruction.starting_location[0] == 8 and instruction.starting_location[1] == 0):
            ...
            loader.add_pending_loads(current_container.description)
            ...
            loader.add_offloads(current_container.description)
